describe('GameFlowStarshipsCardsSuccessPath', () => {
  beforeEach(() => {
    cy.visit('/');
  });
  ...
  it('should navigate to card type selection and start the game', () => {
    ...
    cy.getBySel('select-option-starships').contains('Starships').click();

    cy.intercept('GET', 'https://swapi.dev/api/starships').as('res');
